...
export function LanguageSelect({ value, onChange }: { value: Language; onChange: (v: Language) => void }) {
  ...
  return (
    <Select value={value} onValueChange={(v) => onChange(v as Language)}>
        ...
        <div className="flex items-center gap-1.5">
          ...
          <SelectValue>
            <span className="flex items-center gap-1">
              ...
              <span className="text-secondary">{selected?.label}</span>
            </span>
          </SelectValue>
        </div>
      </SelectTrigger>
      <SelectContent className="bg-card/95 border-secondary/40 backdrop-blur-md z-[1000] max-h-[300px]">
        {LANGUAGES.map((l) => (
          <SelectItem key={l.value} value={l.value} className="text-[9px]">
